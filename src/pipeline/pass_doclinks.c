...
static int doclink_flush(doclink_ctx_t *dc) {
    ...
    for (int i = 0; i < dc->ref_count; i++) {
        ...
        snprintf(props, sizeof(props), "{\"strategy\":\"%s\",\"confidence\":%.2f,\"count\":%d}",
    ...
    if (dc->truncated) {
        ...
        snprintf(cap_buf, sizeof(cap_buf), "%d", DOCLINK_MAX_REFS);
...
int cbm_pipeline_pass_doclinks(cbm_pipeline_ctx_t *ctx) {
    ...
    char buf2[CBM_SZ_16];
    char buf3[CBM_SZ_16];
    snprintf(buf1, sizeof(buf1), "%d", md_edges);
    cbm_log_info("doclinks.strategy", "name", "markdown", "edges", buf1);
    snprintf(buf2, sizeof(buf2), "%d", sh_edges);
    cbm_log_info("doclinks.strategy", "name", "shell", "edges", buf2);
    snprintf(buf3, sizeof(buf3), "%d", md_edges + sh_edges);
